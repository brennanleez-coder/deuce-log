...
interface SessionStatsDialogProps {
  ...
  export default function SessionStatsDialog({
    ...
    useEffect(() => {
      ...
      // 1) Get the per-opponent stats
      const stats = getHeadToHeadForSession(sessionId, name);
      console.log("Head-to-head stats received:", stats)
      setHeadToHead(stats);
      ...
      // 3) Side bet wins/losses for the entire transaction list or just the session?
      // If you only want session side-bet stats, you can filter getSessionTransactions(sessionId).
      // For now, let's do session-based side bet stats from the entire transaction list, but filtered:
      ...
      // You could also do session Settlement if needed
